...
movePaddle = do (batp, ballp, ballv, w) <- get
                ...
                case arrow of
                   Nothing -> return ()
                   Just a -> case a of
                                L -> if batp == 0 then return () else put ((batp - 5), ballp, ballv, w)
                                R -> if batp == 730 then return () else put ((batp + 5), ballp, ballv, w)
                                _ -> return ()
...
batCollision r@(batp, (bx,by), (bvx,bvy), w) = if (round by == 220) && (signum(bvy) > 0) then 
                                                 (if bx >= fromIntegral (batp-10) && bx < fromIntegral (batp+70) then (batp, (bx,by), (((2.0/70.0) * (bx - fromIntegral batp)) - 1,-bvy),w)
                                                                                                           else r) 
                                                                                       else r
...
printGame = do (batp, ballp, ballv, w) <- get
               ...
               -- printing bat
               lift $ drawInWindow w (polygon [(batp-5, 220),(batp-5,230),(batp+65,230),(batp+65,220)])
