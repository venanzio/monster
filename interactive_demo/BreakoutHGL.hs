...
calculateBounce :: Double -> Double -> Double -> (Double, Double)
calculateBounce bx cx width = (ballSpeed * sin (a*offset), (-ballSpeed) * cos (a*offset))
...
batCollision r@(batp, (bx,by), (bvx,bvy), blockps, w) = if (round by > 220 && round by <= 230) && (signum (bvy) > 0) then 
                                                        (if bx >= fromIntegral (batp-10) && bx < fromIntegral (batp+70) then (batp, (bx,220), calculateBounce bx (fromIntegral batp) 60.0,blockps,w)
                                                                                                                        else r) 
                                                                                                  else r

blockCollisions :: GameState -> GameState
blockCollisions r@(batp, (bx,by), (bvx,bvy), [], w)          = r
blockCollisions r@(batp, (bx,by), (bvx,bvy), (b:blockps), w) = case singleBlockCollision (bx,by) b of 
...
singleBlockCollision :: (Double, Double) -> (Double, Double) -> Maybe (Double,Double) 
singleBlockCollision (bx,by) (blx, bly) = if (bx > blx - 1 && bx < blx + 21) then
                                             if (by > bly && by < bly + 10) then Just (calculateBounce bx blx 20.0)
                                                                                else Nothing
                                                                         else Nothing
