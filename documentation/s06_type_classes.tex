...
In order to define the function application method, we need several binary operations applying functions to arguments inside products, $M$-actions, $M$-monsters.
...
This is lifted to $\alift{\pappl}$, which distributes down throgh the components of the applicative values $\phi$ and $\sigma$, and finally guarded by the constructor $\mcons$.
...
\begin{lemma}\label{lemma:appl_pure_id}
$$
(\apure\,\id) \appl m_a = m_a
$$
\end{lemma}

\begin{lemma}\label{lemma:appl_pure_app}
$$
\apure\,(f\,a) = (\apure\,f) \appl (\apure\,a)
$$
\end{lemma}

\begin{lemma}\label{lemma:appl_app_pure}
$$
m_f \appl (\apure\,a) = (\apure\,(\lambda f. f\,a)) \appl m_f
$$
\end{lemma}

\begin{lemma}\label{lemma:appl_assoc}
$$
m_g \appl (m_f \appl m_a) = (m_g \alift{\comp} m_f) \appl m_a 
$$
\end{lemma}

In summary, the monster operator is applicative when the underlying ``monad'' is.
\begin{theorem}
...
One solution to this is to only allow the underlying monad to be idempotent \cite{idempotent_monads}. Taking the diagonal here would work fine, since with an idempotent monad, any order in which you join actions results in the same action. This is quite a strong constraint however, and it would be interesting if there is a weaker one.
