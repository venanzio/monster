...
$$
We can do this by proving directly that the heads must be equal, $p_h:h_f(x) = h_g(x)$, and invoking the {\em coinduction hypothesis} $H$ for the tails: $\cdots$ \vcomment{Not quite: to be made more precise}.




 assume we have a class $\cal S$ of streams: $\cal S$ is a subset of $\stream{A}$, maybe defined by a predicate.
Suppose $\cal S$ is closed under tail, that is, whenever $\sigma \in \cal S$, then also $\tail\,\sigma \in \cal S$. 
We can prove that all elements of $\cal S$ satisfy a certain property $P$ by coinduction: Give a name to the theorem we want to prove:
$$
H : \forall \sigma \in \cal S, P\,\sigma.
$$
The proof takes an element $\sigma\in \cal S$, which can be resolved into head and tail: $\sigma = a_0\scons \sigma'$.
