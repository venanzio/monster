...
Another common operation on trees however, tree-sort, cannot be implemented in the same way, as there is no 'root' label - each sub-tree can be sorted with respect to it's first branch, but the branches at the same level are incomparable. This is assuming we have a binary tree, as an arbitrarily branching tree cannot be sorted like this in any case.
