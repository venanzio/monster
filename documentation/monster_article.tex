...
By instantiating with various monads, you can form well-known data types, or variations of these. This has provided good insight into what kinds of operations are possible to perform on monadic streams in general.
...
\codata\;
\mathsf{BLTree}(A):\set\\
\quad \mathsf{collect} : \lst{A \times \mathsf{BLTree}(A)} \rightarrow \mathsf{BLTree}(A)\\
\quad \nil : \mathsf{BLTree}(A).
\end{array}
$$
...
Standard trees operations such as sorting and traversals are different for branch-labelled trees. Branch-labelled trees in general are more useful for applications where you care only about paths down a tree, or transitions between states rather than the states themselves. 
...
Signal functions/mealy machines - reader monad \\
  - providing an input produces an output and another function, which is \\
...
\vcomment{This is very badly explained: needs more extensive clarification.}
