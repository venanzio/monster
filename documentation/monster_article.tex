...
transformMS f s = MCons $ fmap (\(h,t) -> f h t) (uncons s)
...
\subsubsection{Functor Law Proofs}
...
= \s -> MCons $ fmap (\(h,t) -> (\a s -> (id a, fmap id s)) h t) (uncons s)
-- application of `(\a s -> (id a, fmap id s))` to `h t`
= \s -> MCons $ fmap (\(h,t) -> (id h, fmap id t)) (uncons s)
-- take as assumption that `fmap id t == id t` - coinductive hypothesis?
= \s -> MCons $ fmap (\(h,t) -> (id h, id t)) (uncons s)
-- definition of `id`
= \s -> MCons $ fmap (\(h,t) -> (h,t)) (uncons s)
-- definition of `id`
= \s -> MCons $ fmap id (uncons s)
-- `uncons` returns an element in a functor, so `fmap id == id` in this case
= \s -> MCons $ id (uncons s)
-- application of `id`
= \s -> MCons $ (uncons s)
-- MCons and uncons are inverses
= \s -> id s 
= id
\end{haskell}
...
fmap f s = transformMS (\ a s -> (f a, fmap f s)) s
         = MCons $ fmap (\ (h,t) -> (\a s -> (f a, fmap f s) h t)) (uncons s)
         = MCons $ fmap (\ (h,t) -> (f h, fmap f t)) (uncons s)
         = MCons $ fmap tr(f) (uncons s)
\end{haskell}

We also use the fact that the monster constructor \verb+MCons+ and the function \verb+uncons+ that removes it are inverse of each other.

\begin{haskell}
fmap (f . g) = transformMS (\a s -> (f . g) a, fmap (f . g) s)
-- definition of `transformMS`
= \s -> MCons $ fmap (\(h,t) -> (\a s -> ((f . g) a, fmap (f . g) s)) h t) (uncons s)
-- application of `(\a s -> ((f . g) a, fmap (f . g) s))` to `h t`
= \s -> MCons $ fmap (\(h,t) -> ((f . g) h, fmap (f . g) t)) (uncons s)`
-- take as assumption that `fmap (f . g) t == (fmap f . fmap g) t` - coinductive hypothesis?
= \s -> MCons $ fmap (\(h,t) -> ((f . g) h, (fmap f . fmap g) t)) (uncons s)
-- By fuctoriality of pairing in both components
= \s -> MCons $ fmap (tr(f) . tr(g)) (uncons s)
-- By functoriality of the underlying operator m
= \s -> MCons $ (fmap tr(f)) . (fmap tr(g)) (uncons s)
-- MCons and uncons are inverses
= \s -> MCons $ fmap tr(f)) $ uncons $ MCons $ fmap tr(g) (uncons s)
= \s -> MCons $ fmap tr(f)) $ uncons $ fmap g s
= \s -> fmap f $ fmap g s
= fmap f . fmap g
\end{haskell}
...
instance Comonad w => Comonad (MonStr w) where
  ...
  duplicate s = MCons $ fmap (\(h,t) -> (s, duplicate t)) (uncons s)
\end{haskell}
...
= \ms -> MCons $ fmap (\(h,t) -> ((extract . headMS) ms, id t)) (uncons ms)
