...
First, we need the equality of the heads of the output: if $x\sim y$, then $h_f(x) = h_g(x)$.
Second, we need that $\sim$ is preseved under taking the tail functions for $f$ and $g$: if $x\sim y$, then $t_f(x) \sim t_g(x)$; so that we can now apply $H$ to obtain that $f(t_f(x)) = g(t_g(x))$.

A relation $\sim$ satisfying these two properties is called a {\em bisimulation} between the coalgebras $\langle h_f, t_f\rangle$ and $\langle h_g, t_g\rangle$.
...
It is clear that these are exact inverses of each other. 
