...
\ccomm{I don't exactly understand this - shouldn't $f\,x = g\,x$ be $f\,x = g\,y$? Shouldn't this also be the case in all of the equations like  '$x\bisim y$, then $h_f(x) = h_g(x)$' =>  '$x\bisim y$, then $h_f(x) = h_g(y)$' in the next paragraph?}
...
\ccomm{The introduction of the new notation in the next bit is a little abrupt - it is fairly self-explanatory but clarification might be needed}
...
In practice the coinduction principle is applied by recursively invoking the statement that we want to prove as our {\em coinduction hypothesis}, as long as it is {\em guarded by constructors} in the sense that it is deployed  to prove the statement on the components of the initial \ccomm{maybe non-recursive instead?} terms.
