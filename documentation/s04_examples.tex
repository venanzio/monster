...
This function builds a QuickCheck property, which validates whether the drop function (removal of the first $n$ elements) on lists and Maybe-monsters are (extensionally) equivalent. This makes use of the isomorphism, allowing these types to be freely converted between in order to check for equality.
...
When instantiating monsters with the list monad, we get the type of branch-labelled trees. These are a variation on the usual type of Rose trees, where in this case the \emph{edges} of the trees contain values, not the nodes or leaves.
...
There is a close correspondence \ccomm{maybe an equivalence?} between these two types, indicated by these natural transformations:
\begin{haskell}
...
Using this function lets you define dependent processes separately and then combine them afterwards. 'Dependent' refers to the use of the \verb+ReaderT+ monad transformer: the actions of the dependent process (\verb+outputProc+) are specified by the elements in another process (\verb+inputProc+).
