...
In this section, we assume that the type operator $M$ is an applicative functor, that is, it has methods:
...
This in turn uses a similar operator (which works when $M$ is just a functor) which appends an $M$-action to the front of a monster:
...
We explicitly write the type of identity functions as a subscript.
We consistently use $\sigma$ with subscripts to denote monsters and $m$ with subscripts to denote $M$-actions.
We also use the convention that $\apure$ binds stronger than any of the infix operators that we use, specifically $\appl$, $\alift{\comp}$ and $\alift{\pappl}$ (defined below).


Using the applicative laws for $M$, we can give a simplified characterisation of pure monsters.
 The proofs of this and some of the following technical lemmas are included in the Appendix.
