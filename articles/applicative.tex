...
A consequence of the Homomorphism and Composition Laws is that sequential application of functions can be reduced to a single application of their composition;
if $g:B\rightarrow C$ and $h:A_1\rightarrow A_2 \rightarrow B$, then:
$$
\begin{array}{l}
...
Therefore, in future definitions and proofs, we allow ourselves to give corecursive definitions with that of $\apure_{\stream{M}}\,a$ given in Lemma \ref{lemma:monster_pure}, without justifying them by expansion.
...
This is lifted to $\alift{\pappl}$, which distributes down through the components of the applicative values $m_f$ and $m_a$, and is finally guarded by the constructor $\mcons$.
